...
function moveFile($request, $fileName, $unlink = false, $fileDel)
{
    ...
    if($format != 'txt'){
        echo "File phải có định dạng .txt";
    }
    $name = $file->getClientOriginalName();
    $image = str_random(4) . "_" . $name;
    while(file_exists("lib/keywords/" . $image))
    {
        $txtFile = str_random(4) . "_" . $name;
    }
    if($unlink)
    	unlink("lib/keywords/" . $fileDel);
    $file->move("lib/keywords/", $txtFile);
